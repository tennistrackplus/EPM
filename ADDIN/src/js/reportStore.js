 ...
 *
 * NUEVO — puente para el XLAM (VBA):
 * Además del JSON en Office.context.document.settings (que solo puede leer
 * Office.js, no VBA), el diseño del informe ACTIVO se vuelca también como
 * un CustomXmlPart del propio libro (namespace DRACO_XML_NS). Esa parte SÍ
 * es accesible desde VBA vía Workbook.CustomXMLParts, así que el XLAM lee
 * de ahí en vez de celdas físicas para H10/N10/H12/N12 y las filas de
 * filtros/filas/columnas. Ver syncActiveDesignToCustomXmlPart().
 */
...
(function () {
    ...
    const DRACO_XML_NS = "urn:draco:report-design:v1";
    ...
    async function setActiveReportId(reportId) {
        ...
        await _saveAsync();
        // NUEVO: al cambiar de informe activo, el CustomXmlPart que lee el
        // XLAM tiene que reflejar el diseño del informe recién activado.
        await syncActiveDesignToCustomXmlPart();
    }
    ...
    async function saveDesign(reportId, state) {
        ...
        await _writeReport(reportId, report);
        // NUEVO: refleja el diseño recién guardado en el CustomXmlPart que
        // lee el XLAM (sustituye a lo que antes era escribir en la hoja).
        await syncActiveDesignToCustomXmlPart();
    }
    ...
    /* ---------------------------------------------------------------
     * NUEVO: puente CustomXmlPart para el XLAM (VBA)
     * ------------------------------------------------------------- */
    ...
    function _designToXml(design) {
        const d = design || {};

        const filters = (d.filters || []).map(f =>
            `<Filter Dim="${_escXml(f.dimension)}" Attr="${_escXml(f.realAttribute)}" ` +
            `Value="${_escXml(f.value)}" Jerarquia="${_escXml(f.isHierarchy ? f.name : "")}"/>`
        ).join("");

        const axisXml = (list, tag) => (list || []).map(c =>
            `<${tag} Dim="${_escXml(c[0])}" Attr="${_escXml(c[1])}" Nivel="${_escXml(c[2])}" ` +
            `Jerarquia="${_escXml(c[3])}" Subtotal="${_escXml(c[4])}" Orden="${_escXml(c[5])}"/>`
        ).join("");

        return `<DracoDesign xmlns="${DRACO_XML_NS}">` +
            `<RowsRange>${_escXml(d.rrAddress)}</RowsRange>` +
            `<ColsRange>${_escXml(d.rcAddress)}</ColsRange>` +
            `<RowsStatic>${d.rowsStatic ? "X" : ""}</RowsStatic>` +
            `<ColsStatic>${d.colsStatic ? "X" : ""}</ColsStatic>` +
            `<Filters>${filters}</Filters>` +
            `<Rows>${axisXml(d.expandedRows, "Row")}</Rows>` +
            `<Columns>${axisXml(d.expandedCols, "Col")}</Columns>` +
            `</DracoDesign>`;
    }

    /**
     * Vuelca el diseño del informe ACTIVO como CustomXmlPart del libro, para
     * que el XLAM (VBA) pueda leerlo con Workbook.CustomXMLParts en vez de
     * depender de celdas físicas. Solo puede existir una parte con este
     * namespace a la vez: se borra la anterior (si existe) y, si hay
     * informe activo, se añade la nueva.
     */
    async function syncActiveDesignToCustomXmlPart() {
        const report = getActiveReport();
        try {
            await Excel.run(async (context) => {
                const parts = context.workbook.customXmlParts.getByNamespace(DRACO_XML_NS);
                parts.load("items");
                await context.sync();
                parts.items.forEach(p => p.delete());
                if (report) {
                    context.workbook.customXmlParts.add(_designToXml(report.design || {}));
                }
                await context.sync();
            });
        } catch (e) {
            console.error("ReportStore: no se pudo sincronizar el CustomXmlPart de diseño para el XLAM.", e);
        }
    }
    ...
    window.ReportStore = {
        ...
        getReportGrid,
        syncActiveDesignToCustomXmlPart
    };
